...
class Scene(RBC):
    ...
    def _parallelize(
        ...
        offset_x = (np.arange(self._B) % self.n_envs_per_row) * self.env_spacing[0] # original: (np.arange(self._B) // self.n_envs_per_row) * self.env_spacing[0]
        offset_y = (np.arange(self._B) // self.n_envs_per_row) * self.env_spacing[1] # original: (np.arange(self._B) % 
